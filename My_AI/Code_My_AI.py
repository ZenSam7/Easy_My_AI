...
class AI:
    ...
    def learning(self, input_data: List[float], answer: List[float],
        ...
        for weight, layer_answer in zip(self.weights[::-1], answers[::-1]):
            ...
            print(delta_weight.T.shape, layer_answer.shape)
            gradient = delta_weight.T.dot(layer_answer)
            ...
            if self.number_disabled_weights > 0:
                ...
                gradient = np.multiply(dropout_mask, gradient)

            # Изменяем веса
            print(weight.shape, gradient.T.shape)
            weight -= self.alpha * gradient.T

            # "Переносим" градиент на другой слой (+ умножаем на производную)
            # print(delta_weight.shape, weight.T.shape)
            delta_weight = delta_weight.dot(weight.T)
            # print(delta_weight.shape)
            delta_weight = self.what_act_func(layer_answer.T, True).dot(delta_weight)
