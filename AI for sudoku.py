...
#################################################################
#################################################################
"""ЕСЛИ ЧТО, ВЕСЬ КОД ДЛЯ СУДОКУ СКОПИРОВАН ИЗ ИНТЕРНЕТОВ
    (Да и вообще лучше всернуть все эти ненужные функции)"""
import Code_My_AI
import numpy as np
...
def make_holes_in_sudoku(sudoku, how_many_holes):
    """Заменяем рандомные клетки судоки нулями
        (Если что, дырки могут накладываться, и реальное количество дыр может быть меньше)"""
