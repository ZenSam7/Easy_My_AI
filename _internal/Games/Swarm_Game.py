...
class World:
    def __init__(
        self, window_width, window_height, amount_sources, amount_agents, settings
    ):
        """ai_settings: {architecture, add_bias_neuron, alpha, angle_delta}"""
        # Добавляем Источники
        self.all_sources = [
            Source(window_width, window_height) for _ in range(amount_sources)
        ]

        # Добавляем Агентов
        self.all_agents = [
            Agent(window_width, window_height, settings) for _ in range(amount_agents)
        ]
    ...
    def draw(self):
        ...
        for source in self.all_sources:
            pygame.draw.circle(self.window, source.color, source.coords, source.size)

        # Агенты
        for agent in self.all_agents:
            pygame.draw.circle(self.window, agent.color, agent.coords, agent.size)
...
class Agent:
    def __init__(self, window_width, window_height, settings):
        ...
        # Создаём ИИшки
        self.ai = AI()
        self.ai.create_weights(
            settings["architecture"], add_bias=settings["add_bias_neuron"]
        )
        self.ai.main_act_func = self.ai.kit_act_funcs.tanh
    ...
    def move(self, agents_coords):
        ...
        ai_angle = (
            self.ai.predict([*self.coords, *nearest_neighbor])[0] / self.angle_delta
        )
    ...
    def learn(self, objects_coords):
        ...
        answer_angle = (
            math.atan2(
                self.coords[1] - nearest_odject[1], self.coords[0] - nearest_odject[0]
            )
            / math.pi
        )
        # Изменение угла
